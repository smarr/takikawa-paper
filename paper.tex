...
\begin{abstract}
One form of type checking used in gradually typed language is \emph{transient type checking}: when ever an object 'flows' through code with a type annotation, the object is dynamically checked to have the methods required by said annotation.

Though na\"ive implementations of this have a high runtime overhead, just-in-time compilation and optimisation in virtual machines can eliminate much of this overhead.  Unfortunately
the improvement is not uniform: while most type checks can be optimised
away, some will significantly decrease a program's
performance, and some may even increase it.  In this paper we investigate how benchmark results can be used to identify which type checks have the greatest effects on performance.
...
TypeScript
\citep{typeScriptECOOP}), here
%
all types are erased before execution, limiting the benefit of
types to statically typed portions of programs, whilst not providing 
...
, these offer first-order semantics, by checking
that an object's type constructor or supported methods match
any explicit type annotations that the object flows through.
%%%WRONG kjx
...
and ``proxies'' (provided by Reticulated Python), these
...
Unfortunately, most gradual system with run-time semantics
(as opposed to type erasure as in TypeScript) currently
imposes a significant run-time performance overhead to provide such
...
where the object receiving the request decides which they return to the point where execution first encountered the lambda \citep{bluebook}.  In
...
Critically, in Grace, declarations, methods can be annotated with types.
As Grace is designed to support a variety of teaching methods, implementation of Grace are free to check such type annotations  statically, dynamically, or not at all.
 Thus the type system of Grace is intrinsically gradual:%
%
...
and is the default type when type annotations are omitted.
...
In Grace, types specify a set of method signatures that an object must provide. Thus a type expresses the requests an object can respond to, for example whether a particular accessor is available, rather than a location in a class hierarchy.

\paragraph{Moth's Transient Type Checking}
Moth's implementation of transient type checks however are only only first-order, as such it only dynamically checks that an object has methods of the same name and arity as are required by a type, however any argument and return types of such methods are not checked.

In particular, Moth performs such type checks are run time:
\begin{itemize}
...
Moth checks that it's value (\code{o}) still has a \code{three} method;
...
Including the 14 benchmarks of the \AWFY
benchmark suite \cite{Marr2016}: Bounce, CD, DeltaBlue, GraphSearch, Havlak, Json, List, Mandelbrot, NBody, Permute, Richards, Sieve, Storage and Towers benchmarks. We also used benchmarks taken from the gradual-typing literature.
Our goal was to complement our benchmarks with additional ones that are used for similar experiments and can be ported to Grace.
...
and selected 9, benchmarks that have been used by multiple papers.
...
allows programmers to choose whether each individual declaration should be type-checked: so here $N$ is the number of type annotations in the program,
so checking an entire lattice for even a moderately sized benchmark would be infeasible.  Vitousek et al.\ modified the Takikawa protocol for
...
To investigate Moth's support for transient type checks further, we carried out a similar approach to Takikawa-Vitousek evaluation: for each benchmark we generated 100 partially typed versions (or less if the benchmark was too small). We did an even split so that for each $i \ge 1$ and $i$ < $N$ (where $N$ is the total number of type annotation), we generated roughly the same number of configurations with $i$ type annotations. We used Robert Floyd's sampling algorithm \cite{Bentley:1987:PPS:30401.315746} to randomly choose the type annotations each configuration contained, and we ensured that no duplicate configurations where generated. In addition to these, we tested fully untyped and typed versions, for a total of 102 configurations per benchmark, (or 97 in the case of Storage, since it only has 10 type annotations).

As we did in our previous work \cite{roberts-and-co-ecoop-2019} which used the same set of benchmarks, to account for the complex warmup behaviour of modern systems\citep{Barrett:2017:VMW} as well as
the non-determinism caused by \eg garbage collection and cache effects, we ran each benchmark for 1000 iterations in the same invocation of Moth, and used cut off of 350 iterations for warmup, \ie we discard the first 350 iterations, since they showed signs of compilation.

Though outliers, caused by \eg garbage collection, remain visible in the plots for each individual configuration, the largest 95\% confidence interval we obtained (over the mean time after warmup) for any of experiments was $\pm3.53\%$ (for the PyStone benchmark). 

All experiments were executed on a Windows 10 machine
running Fedora 30 within Windows Subsystems for Linux (version 1). The machine has an Intel Core i7-6800K @3.40GHz, with 6 cores, for a total of 12 hyperthreads. We used ReBench 1.0 \citep{ReBench:2018}, Java 1.8.0\_191 Graal 0.43.
...
To enable reproductions, the scripts we used to generate and run our experiments, including the source code for all the configurations tested, is available online.\footnote{
...
\begin{figure*}
	...
	\caption{Graphs of (at most) 102 configurations in the typing latices for each benchmark. Time is measured as the mean of the 351\textsuperscript{st} to the 1,000\textsuperscript{th} benchmark iteration under a single invocation of Moth (lower is better).}
	\label{f:full}
\end{figure*}
The results for the performance of our sample of typing lattice configurations are presented in figure~\ref{f:full}. Following \cite{vitousek-transient-arXive-2019}, the points on each graph show the average execution if each individual configuration. The x-axis represents the proportion of type annotations for each configuration, with the left and right most points showing the times for the fully untyped and typed configurations (respective). The execution time (in milliseconds) is shown on the left y-axes, and time relative to the fully untyped configuration is shown on the right y-axes.

Most of the graphs are mostly horizontal lines: indicating that the performance effect of including type annotations is negligible. However the plots for CD and Richards show a roughly linear increase: i.e. adding more type annotations to the program increases performance linearly. The plots for Go, Permute, DeltaBlue, and Storage even show \emph{decreases}: i.e. adding more type annotations can actually \emph{increase} performance. Of particular note is that some of the graphs appear to show bimodal performance profiles (Permute, Storage, Snake, Towers, and Richards), or in the case of List, trimodal.
