...
In particular, Takikawa protocol evaluations often show that how simply adding more type annotations does not always produce a uniform effect on performance. Here we adapt their approach in order to identify individual type annotations that may be responsible for performance effects.
...
We added type annotations to every benchmark, aiming to use the most appropriate (most specific) annotation for each declaration.
%KJX: I changed it too because I though that was also misleading.
