...
where the object receiving the request decides which
%kjx reverted cos edited version made no sense.
%for this paper, we could just delete the comments about blocks if you like.
method to run, control structures are built out of blocks (essentialy lambda expressions), and return statements are ``non-local'', returning from the lexically enclosing method \citep{bluebook}.
%
In
...
Though outliers remain visible in the plots for each individual benchmark, the largest 95\% confidence interval we obtained for the arithmetic mean of any of the results for any of our experiments was $\pm3.53\%$ (for the PyStone benchmark). 
